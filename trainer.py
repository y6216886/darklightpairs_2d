...
class Trainer(object):
    ...
    def forward(self, images, leftlabels=None, rightlabels=None):
        ...
        # print("label", labels)
        if leftlabels is not None:
            l_loss = self.criterion(output[:,0], leftlabels)
        if rightlabels is not None:
            r_loss = self.criterion(output[:,1],rightlabels)
            loss = l_loss+r_loss
        else:
            loss = None

        return output[:, 0], output[:, 1], loss
    ...
    def train(self, epoch, train_loader):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            #left#
            #####
            left_labels = generateTarget(images, left_label)
            ...
            # right#
            #####
            right_labels = generateTarget (images, right_label)
            # print ("left label", right_labels)
            ...
            l_output, r_output, loss = self.forward (images_var,left_labels_var, right_labels_var)
            # print("loss",loss)
            # print("loss.size[0]",loss.data[0])
            # print("loss.size", loss.data)
